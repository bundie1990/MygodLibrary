...
namespace Mygod.Windows.Dialogs
{
	...
	public class TaskDialogViewModel : IActiveTaskDialog, INotifyPropertyChanged
	{
			...
			get
			{
				if (_normalButtons == null)
				{
					...
					if (CommandLinks.Count == 0
						...
						&& (options.CustomButtons == null || options.CustomButtons.Length == 0)
						&& options.Buttons == TaskDialogButtons.None)
					{
						_normalButtons = new List<TaskDialogButtonData>();
						_normalButtons.Add(new TaskDialogButtonData(
							(int)TaskDialogButtons.Close,
							TaskDialogButtons.Close.ToString(),
							NormalButtonCommand,
							true, true));
					}
					else if (RadioButtons.Count > 0)
					{
						_normalButtons = new List<TaskDialogButtonData>();
						_normalButtons.Add(new TaskDialogButtonData(
							(int)TaskDialogButtons.OK,
							TaskDialogButtons.OK.ToString(),
							NormalButtonCommand,
							true, false));
					}
						...
						_normalButtons =
							(from button in options.CustomButtons
							 select new TaskDialogButtonData(
								...
								button.Contains(TaskDialogButtons.Cancel.ToString()) || button.Contains(TaskDialogButtons.Close.ToString())))
							.ToList();
					}
					else if (options.Buttons != TaskDialogButtons.None)
					{
						int i = 0;
                        TaskDialogButtons comBtns = options.Buttons;
						_normalButtons =
							(from button in Enum.GetValues(typeof(TaskDialogButtons)).Cast<int>()
							 where button != (int)TaskDialogButtons.None
								&& comBtns.HasFlag((TaskDialogButtons)button)
							 select TaskDialog.ConvertCommonButton(
								(TaskDialogButtons)button,
								NormalButtonCommand,
								DefaultButtonIndex == i++,
								(TaskDialogButtons)button == TaskDialogButtons.Cancel || (TaskDialogButtons)button == TaskDialogButtons.Close))
							.ToList();
					}
					else
					{
	    ...
	    {
            get { return options.ExpandedControlText; }
            private set
            {
        ...
        {
            get { return options.CollapsedControlText; }
            private set
            {
			...
			try
			{
				switch (icon)
				{
					...
					case TaskDialogIcon.Shield:
                        if (Environment.OSVersion.Version.Major >= 6) sysIcon = SystemIcons.Shield;
                        else altBmp = new Bitmap(CurrentApp.GetResourceStream("/Resources/shield-" + (isLarge ? 32 : 16) + ".png"));
						break;
				}
